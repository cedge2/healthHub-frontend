...
export function Input({ label, name, type, color, placeholder, register }) {
  ...
  const inputBorderClass = 'border border-grey-400'; 

  return (
    <div className="text-sm w-full">
      <label
        htmlFor={name} 
        className={`block mb-2 ${color ? 'text-black' : 'text-white'}`} 
      >
        {label}
      </label>
      <input
        ...
        className={`w-full bg-transparent text-sm p-4 rounded-lg ${inputBorderClass} ${color ? 'font-light text-black' : 'font-semibold text-white'}`} 
      />
    </div>
  );
}
